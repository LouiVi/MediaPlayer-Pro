...
app.Script( "Home.js" );
app.Script( "About.js" );
app.Script( "Images.js" );
app.Script( "File.js" );
app.Script( "Utils.js" );

//Init some global variables.
var appPath = "/sdcard/MediaPlayer Pro";
var curMenu = "Home";
var curPage = null;
...
function OnStart()
{
app.ShowPopup( app.ListFolder( "/storage/emulated/0/Download/", "*.*", -1) );
    app.CopyFile( "Fonts/Icon.ttf", "/storage/emulated/0/Download/Icon.ttf" );
    app.Wait( 3, false );
    if(app.FileExists( "/storage/emulated/0/Download/Icon.ttf") ) alert("The file was copied!");
	...
	layMain.SetBackColor( "#ffffff" );
	 dsnav = app.CreateDSNav(color.ORANGE_ACCENT_3, "#ffffff","bottom");
  //dsnav.Hide();
  dsnav.SetIcon("home,image,music_note,video_library,settings"," Home, Images, Audio, Videos, Settings")
  //dsnav.SetActiveTab(2)
  dsnav.SetIconSize(21)
  dsnav.SetIconFontFile("/storage/emulated/0/Android/data/com.smartphoneremote.androidscriptfree/files/DroidScript/DS Nav/Fonts/MaterialIcons-Regular.ttf")
  dsnav.SetOnTouch((index,tabname)=>{ app.ShowPopup( index+":"+tabname)})
  dsnav.AddChild([layMain],[layMain],[layMain],[layMain],[layMain])
  dsnav.Set()
  dsnav.Show()
...
function CreateMenuBar()
{
    ...
    layHoriz2.SetBackGradient( color.AMBER_LIGHT_5, color.AMBER_DARK_4, color.AMBER_ACCENT_4 );
...
if(d==0) mt[c] = app.CreateText( mi2[c] ), mt[c].SetMargins(0.01,0.01,0.01,0.01), mt[c].index = c, mt[c].SetOnTouch(mt_OnTouch),mt[c].SetFontFile("Fonts/Icon.ttf"), mv[d].AddChild(mt[c]), mt[c].SetTextSize(24), mt[c].SetTextColor("#ffffff"), mt[c].SetTextShadow(5,0,0,color.AMBER_DARK_4);
if(d==1) mt[c] = app.CreateText( mi[c] ), mv[d].AddChild(mt[c]), mt[c].index = c, mt[c].SetOnTouch(mt_OnTouch),mt[c].SetTextColor("#ffffff"),  mt[c].SetTextShadow(5,0,0,color.AMBER_ACCENT_4),mt[c].SetTextSize(12);
...
mh[self.index].Animate("Rubberband",null, 350);
...
function CreateActionBar()
{
    ...
    layHoriz.SetBackGradient( color.PINK_LIGHT_4, color.PINK_DARK_2, color.PINK_ACCENT_2);
    ...
    txtMenu.SetTextSize( 20 );
    txtMenu.SetTextColor( color.PINK_LIGHT_5 );
txtMenu.SetTextShadow( 7, 2, 2, "#000000" );
    txtMenu.SetOnTouchUp( function(){ChangePage( home, "Home" ),txtMenu.SetText( "[fa-home]"); } );
    ...
    //Create title.
    txtBarTitle = app.CreateText( "MediaPlayer Pro", -1,-1, "Left" );
    txtBarTitle.SetFontFile( "Misc/YoungSerif-Regular.ttf" );
    txtBarTitle.SetMargins(0,0,0,0,"dip");
    txtBarTitle.SetTextSize( 20 );
    txtBarTitle.SetTextColor( color.PINK_LIGHT_5);
 txtBarTitle.SetTextShadow( 7, 2, 2, "#000000" );
    layBarTitle.AddChild( txtBarTitle );
    
        
    //Create search icon.
    txtSearch = app.CreateText( "[fa-bars]", -1,-1, "FontAwesome" );
    txtSearch.SetPadding( 0,10,0,10, "dip" );
    txtSearch.SetTextSize( 20  );
    txtSearch.SetTextColor( color.PINK_LIGHT_5 );
...
function CreateDrawer()
{
	...
	var txtName = app.CreateText( "MediaPlayer Pro",-1,-1,"Bold");
...
//[fa-file-image-o][fa-file-audio-o][fa-file-video-o][fa-cog]
	
    //Add a list to menu layout (with the menu style option).
    var listItems = "Home::[fa-home],Images::[fa-file-image-o],Audio::[fa-file-audio-o],Videos::[fa-file-video-o],Settings::[fa-cog],About::[fa-question-circle],New File::[fa-plus]";
...
function lstMenu_OnTouch( title, body, type, index )
{
    ...
    //Handle page changes.
    curMenu = title;
    if( title=="Home" ) ChangePage( home, title ),txtMenu.SetText( "[fa-home]" )
    //The Line below is a test.
    else if( title=="Images" ) ChangePage( images, title ),txtMenu.SetText( "[fa-home]" )
    else ChangePage( file, title ), txtMenu.SetText( "[fa-arrow-circle-o-left]" );
    ShowFiles();
    //Close the drawer.
    app.CloseDrawer( "Left" );
}
...
function ShowFiles()
{
    ...
    for( var i=0; i<fileList.length && fileList[0]!=""; i++ )
    {
        ...
        list += fileList[i] + ":" + GetThumbnail(fileList[i]);//"::[fa-file]";
    }
...
{
	return JSON.parse(app.ReadFile( appPath + "/" + str + "/" + str + ".json") ).icon;
}